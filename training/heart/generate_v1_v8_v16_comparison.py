"""
V1 vs V8 vs V16 성능 비교 — 발표용 pandas 테이블 + matplotlib 시각화
V16: File-level split + SpecAugment + CosineAnnealing + LabelSmoothing (데이터 누수 수정)
"""
import os
import numpy as np
import pandas as pd
...
# V16 클래스별 성능 (실측)
v16_per_class = {
    'Normal':   {'precision': 0.83, 'recall': 0.82, 'f1': 0.83},
    'Abnormal': {'precision': 0.81, 'recall': 0.83, 'f1': 0.82},
    'Unknown':  {'precision': 1.00, 'recall': 1.00, 'f1': 1.00},
}

ver_labels = ['V1', 'V8', 'V16']
...
df_improvement = pd.DataFrame([{
    ...
    '버전': 'V8->V16 개선',
    'MACC (%)': round((versions['V16']['macc'] - versions['V8']['macc']) * 100, 2),
    'Sensitivity (%)': round((versions['V16']['sensitivity'] - versions['V8']['sensitivity']) * 100, 2),
    'Specificity (%)': round((versions['V16']['specificity'] - versions['V8']['specificity']) * 100, 2),
    'Accuracy (%)': round((versions['V16']['accuracy'] - versions['V8']['accuracy']) * 100, 2),
}, {
    '버전': 'V1->V16 총 개선',
    'MACC (%)': round((versions['V16']['macc'] - versions['V1']['macc']) * 100, 2),
    'Sensitivity (%)': round((versions['V16']['sensitivity'] - versions['V1']['sensitivity']) * 100, 2),
    'Specificity (%)': round((versions['V16']['specificity'] - versions['V1']['specificity']) * 100, 2),
    'Accuracy (%)': round((versions['V16']['accuracy'] - versions['V1']['accuracy']) * 100, 2),
}])
...
class_data = []
for cls_name, cls_metrics in v16_per_class.items():
    class_data.append({
        '클래스': cls_name,
...
df_v16_class = pd.DataFrame(class_data)

print("\n" + "=" * 70)
print("  테이블 1: V1 vs V8 vs V16 핵심 성능 지표")
print("=" * 70)
...
print("\n" + "=" * 70)
print("  테이블 3: V16 클래스별 성능")
print("=" * 70)
print(df_v16_class.to_string(index=False))

# CSV 저장
csv_metrics_path = os.path.join(OUT_DIR, 'v1_v8_v16_metrics.csv')
csv_structure_path = os.path.join(OUT_DIR, 'v1_v8_v16_structure.csv')
csv_class_path = os.path.join(OUT_DIR, 'v1_v8_v16_class.csv')
...
df_v16_class.to_csv(csv_class_path, index=False, encoding='utf-8-sig')
...
fig.suptitle('Heart Sound Classification — V1 vs V8 vs V16 종합 비교',
...
all_versions = ['V1', 'V2', 'V4', 'V5', 'V7', 'V8', 'V9', 'V10', 'V12', 'V13', 'V14', 'V16']
all_macc = [52, 52, 65, 71, 72, 78, 78, 83, 83, 85, 91, 82.4]
...
# V1, V8, V16 강조
highlight_idx = [0, 5, 11]
highlight_colors = [GRAY, ORANGE, CYAN]
highlight_labels = ['V1 (52%)', 'V8 (78%)', 'V16 (82.4%)']
...
# V8->V16 개선폭
ax2.annotate('', xy=(11, 82.4), xytext=(5, 78),
             arrowprops=dict(arrowstyle='<->', color=CYAN, lw=2, ls='--'))
ax2.text(8, 79, '+4.4%p\n(3-Class+FileSplit)', ha='center', fontsize=10,
...
rows = [
    ...
    ('핵심', ['베이스라인', '윈도우 도입', '3-Class + FileSplit\n+ SpecAugment']),
]
...
waterfall_labels = ['V1\n(시작)', 'V1->V8\n윈도우 도입', 'V8->V16\n3-Class+FileSplit', 'V16\n(최종)']
waterfall_bottoms = [0, 52, 78, 0]
waterfall_heights = [52, 26, 4.4, 82.4]
...
# ─── Panel 6: V16 클래스별 F1-Score ───
ax6 = fig.add_subplot(gs[1, 2])
style_ax(ax6, 'V16 클래스별 성능', '', '%')

cls_names = ['Normal', 'Abnormal', 'Unknown']
cls_colors = [GREEN, RED, ORANGE]
f1_scores = [v16_per_class[c]['f1']*100 for c in cls_names]
precisions = [v16_per_class[c]['precision']*100 for c in cls_names]
recalls = [v16_per_class[c]['recall']*100 for c in cls_names]
...
chart_path = os.path.join(OUT_DIR, '09_v1_v8_v16_comparison.png')
...
ax_t.axis('off')
ax_t.set_title('V1 vs V8 vs V16 — 핵심 성능 지표 비교 (File-Level Split, No Data Leakage)',
...
row_configs = [
    ...
    ('V16 (Production)', versions['V16'], CYAN),
]
...
improvements = [
    ...
    ('V8->V16 개선', versions['V8'], versions['V16'], GREEN),
    ('V1->V16 총 개선', versions['V1'], versions['V16'], GREEN),
]
...
table_img_path = os.path.join(OUT_DIR, '10_v1_v8_v16_table.png')
