...
// ══════════════════════════════════════════════════════════
//  DSP Filter Constants (Heart BPM Detection)
//  Bandpass 20-200Hz: S1/S2 심음 대역에 집중
//  (기존 112-800Hz → 노이즈 과다 통과로 심음 피크 묻힘 문제 수정)
// ══════════════════════════════════════════════════════════
static constexpr float DSP_NOISE_FLOOR     = 0.0001f;  // 심음은 약한 신호
static constexpr float DSP_THRESHOLD_RATIO = 0.40f;    // 확실한 피크만 감지 (0.25->0.40)
static constexpr float DSP_PEAK_ENV_MIN    = 0.002f;  // envelope 이 값 미만이면 피크 무시 (너무 약한 소리)
static constexpr float DSP_PEAK_ENV_MAX    = 0.015f;  // envelope 이 값 초과하면 피크 무시 (외부 소음/탭)
...
static constexpr float DSP_SIGNAL_VAR_MIN  = 0.000001f; // 더 관대한 신호 판정 (0.00001->0.000001)
static constexpr unsigned long DSP_BPM_TIMEOUT_MS = 4000UL;

// HP 20Hz (2nd order Butterworth @ 8kHz) — S1 저주파 보존
static constexpr float DSP_HP_B0 =  0.98895f;
static constexpr float DSP_HP_B1 = -1.97791f;
static constexpr float DSP_HP_B2 =  0.98895f;
static constexpr float DSP_HP_A1 = -1.97779f;
static constexpr float DSP_HP_A2 =  0.97803f;

// LP 200Hz (2nd order Butterworth @ 8kHz) — 심음 대역 집중 (기존 800Hz)
static constexpr float DSP_LP_B0 =  0.00554f;
static constexpr float DSP_LP_B1 =  0.01109f;
static constexpr float DSP_LP_B2 =  0.00554f;
static constexpr float DSP_LP_A1 = -1.77863f;
static constexpr float DSP_LP_A2 =  0.80080f;
...
// ── BPM 유효 범위: 이 범위 밖 피크 간격은 BPM 계산에서 무시 ──
static constexpr float DSP_BPM_VALID_MIN = 50.0f;    // 최소 50 bpm (간격 1200ms 이상은 무시)
static constexpr float DSP_BPM_VALID_MAX = 180.0f;   // 최대 180 bpm (간격 333ms 이하는 무시)
static constexpr float DSP_RR_MIN_MS    = 60000.0f / DSP_BPM_VALID_MAX;  // = 333ms
static constexpr float DSP_RR_MAX_MS    = 60000.0f / DSP_BPM_VALID_MIN;  // = 1200ms
static constexpr float DSP_BPM_MIN_INIT = 1.0f;
static constexpr float BPM_DISPLAY_MIN  = DSP_BPM_VALID_MIN;
static constexpr float BPM_DISPLAY_MAX  = DSP_BPM_VALID_MAX;
static constexpr float BPM_CHANGE_THRESHOLD = 0.5f;

// ══════════════════════════════════════════════════════════
//  RR (Respiratory Rate) DSP Constants
//  호흡 주기: Shannon energy envelope의 상승 에지 간격으로 측정
//  정상 성인 12~20회/min = 3~5초/cycle
//  Bandpass 100-800Hz (폐음 대역) + Shannon energy (-x^2 * log(x^2))
// ══════════════════════════════════════════════════════════
// RR Bandpass: HP 100Hz + LP 800Hz (2nd order Butterworth @ 8kHz)
static constexpr float RR_HP_B0 =  0.94598f;
static constexpr float RR_HP_B1 = -1.89195f;
static constexpr float RR_HP_B2 =  0.94598f;
static constexpr float RR_HP_A1 = -1.88903f;
static constexpr float RR_HP_A2 =  0.89487f;

static constexpr float RR_LP_B0 =  0.06746f;
static constexpr float RR_LP_B1 =  0.13491f;
static constexpr float RR_LP_B2 =  0.06746f;
static constexpr float RR_LP_A1 = -1.14298f;
static constexpr float RR_LP_A2 =  0.41280f;

static constexpr int   RR_SHANNON_WINDOW   = 160;    // Shannon 단기 스무딩 20ms (8kHz * 0.02)
static constexpr int   RR_SMOOTH_WINDOW    = 4000;   // 이동평균 500ms (8kHz * 0.5)
static constexpr float RR_NOISE_FLOOR      = 0.001f; // Shannon 에너지 최소치 (무음 판별)
static constexpr float RR_THRESHOLD_RATIO  = 0.35f;  // running_max 대비 threshold 비율
...
static constexpr int PATIENT_ROW_HEIGHT = 22;
static constexpr int PATIENT_VISIBLE_ROWS = 5;
...
// AI Result Display & Waiting Animation
static constexpr unsigned long AI_WAIT_TIMEOUT_MS  = 30000UL; // 30초 무응답 → RECHECK 자동 표시
static constexpr unsigned long AI_DOT_ANIM_MS      = 500UL;   // 점 애니메이션 전환 간격
...
static String         aiResultLabel      = "";     // "Normal", "Abnormal", "RECHECK", etc.
static int            aiResultConfidence = 0;

// AI 결과 대기 상태 (녹음 완료 후 서버 응답 전)
static volatile bool  isWaitingAI   = false;  // 서버 응답 대기 중 플래그
static unsigned long  waitingStartMs = 0;      // 대기 시작 시각
static unsigned long  dotAnimMs      = 0;      // 점 애니메이션 타이머
static int            dotAnimStep    = 0;      // 점 단계: 0=".", 1="..", 2="..."
static String         lastAiLabel    = "";     // 마지막 AI 결과 라벨 (미니 표시용)
static int            lastAiConf     = 0;      // 마지막 AI 신뢰도 (미니 표시용)
...
static void dsp_process(int16_t raw16)
{
    ...
    // Peak detection (envelope이 MIN~MAX 범위 안에 있어야 유효)
    if (envelope > threshold
        && envelope > DSP_NOISE_FLOOR
        && envelope >= DSP_PEAK_ENV_MIN
        && envelope <= DSP_PEAK_ENV_MAX
        && _samples_since_peak > DSP_PEAK_REFRACTORY) {

        dsp_beat_count++;
        dsp_beat_flag = true;
        Serial.printf("[DSP] PEAK env=%.5f thr=%.5f rmax=%.5f\n", envelope, threshold, _running_max);
...
//  Shannon energy envelope의 상승 에지(조용->소리) 간격으로 호흡 주기 측정
//  Bandpass 100-800Hz -> Shannon(-x^2*log(x^2)) -> smoothing -> onset detect
// ══════════════════════════════════════════════════════════
...
// Bandpass IIR 상태 (HP 100Hz + LP 800Hz)
static float _rr_hp_x1 = 0.0f, _rr_hp_x2 = 0.0f;
static float _rr_hp_y1 = 0.0f, _rr_hp_y2 = 0.0f;
static float _rr_lp_x1 = 0.0f, _rr_lp_x2 = 0.0f;
static float _rr_lp_y1 = 0.0f, _rr_lp_y2 = 0.0f;

// Shannon 단기 스무딩 순환 버퍼 (20ms)
static float _rr_se_buf[RR_SHANNON_WINDOW] = {0};
static int   _rr_se_idx = 0;
static float _rr_se_sum = 0.0f;

// 장기 이동평균 스무딩 순환 버퍼 (500ms)
...
{
    _rr_hp_x1 = 0.0f; _rr_hp_x2 = 0.0f;
    _rr_hp_y1 = 0.0f; _rr_hp_y2 = 0.0f;
    _rr_lp_x1 = 0.0f; _rr_lp_x2 = 0.0f;
    _rr_lp_y1 = 0.0f; _rr_lp_y2 = 0.0f;

    memset(_rr_se_buf, 0, sizeof(_rr_se_buf));
    _rr_se_idx = 0;
    _rr_se_sum = 0.0f;
...
static void rr_process(int16_t raw16)
{
    ...
    // ── Step 1: Bandpass 100-800Hz (HP + LP, 2nd order IIR) ──
    const float hp = RR_HP_B0 * s
                   + RR_HP_B1 * _rr_hp_x1
                   + RR_HP_B2 * _rr_hp_x2
                   - RR_HP_A1 * _rr_hp_y1
                   - RR_HP_A2 * _rr_hp_y2;
    ...
    // ── Step 2: Shannon energy: -x^2 * log10(x^2) ──
    //   큰 진폭(호흡음) 강조, 작은 진폭(무음/노이즈) 억제
    const float x2 = lp * lp;
    float se = 0.0f;
    if (x2 > 1e-10f) {
        const float x2n = (x2 > 1.0f) ? 1.0f : x2;
    ...
    // ── Step 3: 단기 Shannon 스무딩 (20ms) ──
    _rr_se_sum -= _rr_se_buf[_rr_se_idx];
    _rr_se_buf[_rr_se_idx] = se;
    _rr_se_sum += se;
    _rr_se_idx = (_rr_se_idx + 1) % RR_SHANNON_WINDOW;
    const float se_avg = _rr_se_sum / static_cast<float>(RR_SHANNON_WINDOW);

    // ── Step 4: 장기 이동평균 (500ms) — 호흡 주기 추출 ──
    _rr_smooth_sum -= _rr_smooth_buf[_rr_smooth_idx];
    _rr_smooth_buf[_rr_smooth_idx] = se_avg;
    _rr_smooth_sum += se_avg;
    ...
    void onWrite(BLECharacteristic* pCharacteristic) override
    {
        ...
        if (rxValue.startsWith("AI:")) {
            ...
            if (c1 != -1 && c2 != -1) {
                ...
                lastAiLabel        = aiResultLabel;
                lastAiConf         = aiResultConfidence;
                isWaitingAI        = false;
                aiResultPending    = true;  // loop()에서 changeState(AI_RESULT) 호출
                uiNeedsUpdate      = true;
...
// ── AI Result Full-Screen Popup ──
// AI_RESULT 상태에서 drawUI()가 호출하는 전체 화면 팝업
// 버튼(L 또는 R) 누르면 닫힘 — 자동 닫힘 없음
static void drawAiResult()
{
    ...
    const uint16_t typeColor = isHeart ? TFT_RED : TFT_CYAN;

    // 상단 색 바
    tft.fillRect(0, 0, w, 22, typeColor);
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(TFT_BLACK, typeColor);
    tft.drawString(isHeart ? "♥ HEART" : "~ LUNG", cx, 11, 2);

    // 판정 결과 색상
    uint16_t resultColor;
    if (aiResultLabel == "Normal") {
        resultColor = TFT_GREEN;
    } else if (aiResultLabel == "RECHECK") {
    ...
    // 결과 아이콘 (크게)
    const char* icon = (aiResultLabel == "Normal") ? "OK" :
                       (aiResultLabel == "RECHECK") ? "!?" : "!!";
    tft.setTextColor(resultColor, TFT_BLACK);
    tft.drawString(icon, cx, 48, 4);

    // 라벨
    tft.setTextPadding(w);
    if (aiResultLabel.length() > 8) {
        tft.drawString(aiResultLabel, cx, 78, 2);
    } else {
        tft.drawString(aiResultLabel, cx, 78, 4);
    }
    tft.setTextPadding(0);

    // 신뢰도 (RECHECK이면 표시 안함)
    if (aiResultLabel != "RECHECK" && aiResultConfidence > 0) {
        char confBuf[10];
        snprintf(confBuf, sizeof(confBuf), "%d%%", aiResultConfidence);
        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.drawString(confBuf, cx, 115, 4);
        tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
        tft.drawString("confidence", cx, 135, 1);
    } else if (aiResultLabel == "RECHECK") {
        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        tft.drawString("다시 청진해 주세요", cx, 118, 1);
    }

    // 하단 안내
    tft.drawFastHLine(10, h - 35, w - 20, TFT_DARKGREY);
...
void drawUI()
{
    ...
    if (currentState == POPUP) {
        ...
        if (currentMode == MODE_HEART) {
            ...
            // 대기 중 애니메이션 또는 마지막 결과 미니 표시
            if (isWaitingAI) {
                const char* dots[] = {".", "..", "..."};
                tft.setTextColor(TFT_YELLOW, TFT_BLACK);
                tft.setTextPadding(60);
                tft.drawString(dots[dotAnimStep], cx, 68, 2);
                tft.setTextPadding(0);
            } else if (lastAiLabel.length() > 0) {
                char miniBuf[20];
                snprintf(miniBuf, sizeof(miniBuf), "[%s %d%%]",
                         lastAiLabel.c_str(), lastAiConf);
                const uint16_t miniColor = (lastAiLabel == "Normal")  ? TFT_GREEN  :
                                           (lastAiLabel == "RECHECK") ? TFT_ORANGE : TFT_RED;
                tft.setTextColor(miniColor, TFT_BLACK);
                tft.setTextPadding(80);
                tft.drawString(miniBuf, cx, 68, 1);
                tft.setTextPadding(0);
            }
        }
        else if (currentMode == MODE_LUNG) {
        ...
        if (currentMode == MODE_HEART || currentMode == MODE_LUNG) {
            if (selectedPatient == "NONE") {
                tft.drawString("L: MODE | R: RECORD", cx, tft.height() - 15, 1);
            } else {
                tft.drawString("R:REC | R(Hold):DESEL", cx, tft.height() - 15, 1);
            }
        } else {
            tft.drawString("R:Next | R(Hold):SEL", cx, tft.height() - 15, 1);
        }
    }
...
void loop()
{
    ...
    // ── AI 결과 수신 처리 (BLE 콜백에서 flag 설정) ──
    if (aiResultPending) {
        aiResultPending = false;
        changeState(AI_RESULT);
        return;
    }

    // ── AI 대기 중: 점 애니메이션 및 30초 타임아웃 ──
    if (isWaitingAI) {
        // 점 애니메이션 단계 전환
        if ((millis() - dotAnimMs) >= AI_DOT_ANIM_MS) {
            dotAnimStep = (dotAnimStep + 1) % 3;
            dotAnimMs   = millis();
            uiNeedsUpdate = true;
        }
        // 30초 타임아웃 → RECHECK 자동 표시
        if ((millis() - waitingStartMs) >= AI_WAIT_TIMEOUT_MS) {
            isWaitingAI        = false;
            aiResultType       = (currentMode == MODE_HEART) ? "HEART" : "LUNG";
            aiResultLabel      = "RECHECK";
            aiResultConfidence = 0;
            lastAiLabel        = "RECHECK";
            lastAiConf         = 0;
            changeState(AI_RESULT);
            return;
    ...
    // ── AI 결과 팝업 상태: 버튼(L 또는 R) → ACTIVE 복귀 ──
    if (currentState == AI_RESULT) {
        if (eventL != BTN_NONE || eventR != BTN_NONE) {
            changeState(ACTIVE);
        }
        drawUI();
        return;
    ...
    if (currentState == POPUP) {
        ...
        if (!isWaitingAI) {
            if (currentMode == MODE_PATIENT) {
                ...
                // Heart/Lung mode: right short = recording, right long = deselect patient
                if (eventR == BTN_SHORT) {
                    if (deviceConnected) {
                        startRecording();
                    } else {
                    ...
                    tft.fillScreen(TFT_BLACK);
                    tft.setTextColor(TFT_ORANGE, TFT_BLACK);
                    tft.drawString("DESELECTED", tft.width() / 2, tft.height() / 2, 4);
                    delay(PATIENT_SELECT_DELAY_MS);
    ...
    // AI 대기 중(isWaitingAI) 또는 AI 결과 팝업 중(AI_RESULT)에는 파형 그리지 않음
