    ...
    int update() {
        ...
        if (reading == LOW && lastReading == HIGH) {
            ...
            if (pressTime > 0 && !longPressFired && (millis() - pressTime > 1500)) {
                longPressFired = true;
                event = 2; // 1.5초 이상 꾹 누름
            }
        } 
        else if (reading == HIGH && lastReading == LOW) {
            if (pressTime > 0 && !longPressFired && (millis() - pressTime > 50)) {
                event = 1; // 손가락을 완벽히 뗐을 때만 1회 발동!
            }
            pressTime = 0;
        }
        lastReading = reading;
...
class MyServerCallbacks: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer) { 
        ...
        // 화면 표시는 충돌을 막기 위해 loop()에서 안전하게 처리합니다.
    }
...
void setup() {
    ...
    audioTaskHandle = xTaskCreateStaticPinnedToCore(
        audioTask, "AudioTask", AUDIO_TASK_STACK_SIZE, NULL, 2, 
        audioTaskStack, &audioTaskBuffer, 0
    );
}
...
// ══════════════════════════════════════════════════════════
void loop() {
    // 💡 연결 상태 변경 감지를 안전하게 Main Loop에서 처리 (TFT 충돌 완벽 방지)
    ...
    // 💡 버튼 클래스 업데이트 (0: 안 누름, 1: 짧게 누름, 2: 길게 누름)
